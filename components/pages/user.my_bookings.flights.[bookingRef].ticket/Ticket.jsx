...
export default function FlightTicket({ bookingData, qrCodeStr }) {
  const flightData = bookingData.flightSnapshot;
  const passengers = bookingData.passengers;
  const seats = bookingData.seats;
  const flightDate = new Date(+flightData.departure.scheduled);
  const arrivalDate = new Date(+flightData.arrival.scheduled);
  const gate = flightData.departure.gate || "TBD";
  const from = flightData.departure.airport.iataCode;
  const to = flightData.arrival.airport.iataCode;
  const airlineCode = flightData.airlineId.iataCode;
  ...
  async function handleDownload(index, passengerFullName, e) {
    if (e) e.target.disabled = true;
    const originalElement = document.getElementById(`ticket-${index}`);
    ...
    pdf.save(`ticket_${bookingData.bookingRef}_${passengerFullName}.pdf`);
  ...
  function downloadAll(e) {
    ...
    passengers.forEach((passenger, index) => {
      handleDownload(index, `${passenger.firstName}_${passenger.lastName}`);
    });
    ...
    <main className="mx-auto mb-[80px] mt-7 w-[90%] text-secondary">
      ...
      <div className="mx-auto max-w-[840px]">
        {passengers.map((passenger, index) => (
          <div key={index}>
            <div id={`ticket-${index}`} className="mb-3 border p-5">
                ...
                <div className="flex min-h-[350px] flex-col-reverse overflow-hidden rounded-lg border md:flex-row">
                  ...
                  <div className="flex flex-1 flex-col justify-between gap-3 border-l p-3 md:border-0">
                    ...
                    <div className="flex flex-wrap gap-8 text-sm">
                      ...
                      />
                      <TicketDetail icon={doorMint} label="Gate" value={gate} />
                      <TicketDetail
                    ...
                    <div className="flex items-end justify-between text-sm">
                      <div>
                        <h2 className="text-2xl font-bold">{airlineCode}</h2>
                        <p className="font-medium opacity-60">{flightNumber}</p>
                      </div>
                      <Image
                        alt="barcode"
                        width={70}
              ...
              <div className="mt-8">
                ...
                <ul className="list-disc pl-[16px] text-[0.875rem] leading-[1.25rem]">
                  ...
                  <li className="mb-[16px]">
                    If the original payment card cannot be presented at check-in
                    or ticket collection, Golobe reserves the right to deny
                    boarding or require payment with an alternative method.
                    Stored payment information is managed in a PCI-DSS compliant
                ...
                <ul className="list-disc pl-[16px] text-[0.875rem] leading-[1.25rem]">
                  ...
                  <li className="mb-[16px]">
                    Changes to booking details may incur additional charges and
            ...
            <div className="flex justify-end">
              <Button
                ...
                onClick={(e) =>
                  handleDownload(
                    index,
                    `${passenger.firstName}_${passenger.lastName}`,
                    e,
                  )
                }
              >
