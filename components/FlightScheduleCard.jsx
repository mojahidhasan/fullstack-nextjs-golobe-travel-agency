    ...
    <div className={ cn(" shadow-lg rounded-[12px] bg-white px-[24px] py-[32px] shadow-small", className) }>
      { variant === "book" && (
        <div className="mb-[24px] flex justify-between font-bold">
          <h3 className="font-tradeGothic text-[1.5rem]">{ airplane.model }</h3>
          <p className="text-[2rem] text-tertiary">${ price.total }</p>
        </div>
      ) }
      <div className="mb-[24px] flex justify-between font-bold">
        <h4 className="font-tradeGothic text-[1.25rem]">Depart { formatInTimeZone(+departure.scheduled, timezone, "eee, MMM d") }</h4>
      ...
      <div className="grid grid-rows-5 items-center gap-[40px] md:flex md:justify-center lg:gap-[80px]">
        ...
        <div>
          <span className="font-semibold lg:text-[1.5rem]">{ formatInTimeZone(+arrival.scheduled, timezone, "h:mm aaa") }</span>{ " " }
