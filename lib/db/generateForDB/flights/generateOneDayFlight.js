...
export default function generateOneDayFlight(
  ...
  for (let airline of airlines) {
    for (const route of airline.operatingRoutes) {
      ...
      const seatAvailability = choosingAirplane.seats.map((seat) => {
        return {
          seatNumber: seat.seatNumber,
          class: seat.class,
          isAvailable: true,
          reservation: {
            reserveDurationType: null,
            reservedBy: null,
      ...
      let reductionFactorForDuration = lerp(
        0,
        0.5,
        normalize(
          totalFlightDuration,
      ...
      priceReductionMultiplierPecentage = reductionFactorForDuration;
      ...
      const currentRouteAllFlights = flightTimes.map((time) => {
        ...
        if (isStopFlight) {
          ...
          const findOriginAirportToRandomAirportRoute =
            airline.operatingRoutes.find((route) => {
              return (
          ...
          const findRandomAirportRouteToArrival = airline.operatingRoutes.find(
            (route) => {
              return (
          ...
          const reductionFactorForStopover = 0.2;
          reductionFactorForDuration = lerp(
            0,
            0.5,
            normalize(
              totalFlightDuration,
          ...
          priceReductionMultiplierPecentage = combineMultipliers([
            reductionFactorForDuration,
            reductionFactorForStopover,
          ]);

          function combineMultipliers(multipliers) {
            return 1 - multipliers.reduce((acc, m) => acc * (1 - m), 1);
          }
          ...
          stopover = {
            ...
            departure: {
              ...
              scheduled: addMinutes(
                departureTime,
                departureToStopageFlightDuration + totalLayoverDurationInMinutes
              ).getTime(),
              terminal: Math.floor(Math.random() * 4),
              gate: Math.floor(Math.random() * 4),
            },
        ...
        return {
          ...
          expireAt: subMinutes(departureTime, 30), // Flight expires 30 minutes before departure, can not be cancelled or booked after that
        };
      });
