 ...
 * @param {object[]} airlineFlightPrices - array of airline flight price objects
...
export default function generateOneDayFlight(
  ...
  for (let airline of airlines) {
    //available routes for the airline
    ...
    for (const route of routes) {
      ...
      const currentRouteAllFlights = flightTimes.map((time) => {
        ...
        const currentRoutePrice = airlineFlightPrices.find((route) => {
          return (
            route.airlineCode === airline.iataCode &&
            route.departureAirportCode === departureAirportId &&
            route.arrivalAirportCode === arrivalAirportId
          );
        });

        let flightPrice = structuredClone(currentRoutePrice);
        Object.entries(flightPrice.discount).forEach(([key, value]) => {
          flightPrice.discount[key] = {
            type: "percentage", // percentage from base price
            amount: value.amount
...
function combineDiscounts(percentages) {
  const finalPriceRatio = percentages.reduce(
    (acc, p) => acc * (1 - p / 100),
    1,
  );
  return (1 - finalPriceRatio) * 100;
}
