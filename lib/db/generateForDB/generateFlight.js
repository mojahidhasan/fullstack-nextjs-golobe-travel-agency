...
export default async function generateOneDayFlightSchedule(
  ...
  const totalFlightsPerRoute = flightTimes.length; // 3 flightsArr per day per route
    ...
    for (let destination of airports) {
      ...
      for (let airline of airlines) {
        ...
        const s = airplane.seats.map((e) =>
          e._id instanceof mongoose.Types.ObjectId ? e._id.valueOf() : e._id
        );
        const seatsData = seats.filter((el) =>
          s.includes(
            el._id instanceof mongoose.Types.ObjectId
              ? el._id.valueOf()
              : el._id
          )
        );
