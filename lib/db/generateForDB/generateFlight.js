    ...
    for (let destination of airports) {
      ...
      for (let airline of airlines) {
        ...
        const s = airplane.seats.map((e) =>
          new mongoose.Types.ObjectId(e.toString()).toString()
        );
        const seatsData = seats
          .filter((el) =>
            s.includes(
              new mongoose.Types.ObjectId(el._id.toString()).toString()
            )
          )
          .map((el) => {
            return {
              ...el,
              _id: new mongoose.Types.ObjectId(el._id.toString()),
              airplaneId: new mongoose.Types.ObjectId(el.airplaneId.toString()),
            };
          });
          ...
          let stops = [
            {
              ...
              airplaneId: new mongoose.Types.ObjectId(airplane._id.toString()),
