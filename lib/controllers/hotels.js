...
export async function getHotels(searchState, options = {}) {
  ...
  try {
    ...
    const filteredByAvailability = hotels.map(async (hotel) => {
      const availableRooms = await Promise.all(
        hotel.rooms.filter(async (room) => {
          return await isRoomAvailable(
            room?._id || room,
            checkIn,
            checkOut,
            guests,
      ...
      if (totalCapacity < guests || availableRooms.length < rooms) return;
    ...
    hotels = (await Promise.all(filteredByAvailability)).filter(Boolean);
