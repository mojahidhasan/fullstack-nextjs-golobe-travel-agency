  ...
  flightResults = await Promise.all(
    flightResults.map(async (flight) => {
      ...
      const availableSeats = flight.segmentIds.map(async (segment) => {
        const seats = await getAvailableSeats(segment._id, flightClass);
        return {
...
export async function getAvailableFlightDateRange() {
  try {
    ...
    return {
      ...
      data: {
        from:
          firstFlight?.expireAt?.getTime() ||
          addYears(new Date(), 100).getTime(),
        to: lastFlight?.expireAt?.getTime() || -1,
      },
    };
  } catch (e) {
    console.log(e);
...
export async function getReservedSeats(segmentId, seatClass) {
  try {
    const flightSeats = await getManyDocs("FlightSeat", {
      segmentId: segmentId,
...
export async function getAvailableSeats(segmentId, seatClass) {
  try {
    const flightSeats = await getManyDocs("FlightSeat", {
...
export async function getExpiredTemporarylyReservedSeats(segmentId, seatClass) {
  if (segmentId === undefined) throw Error("FlightNumber is required");

  try {
    const reservedSeats = await getManyDocs("FlightSeat", {
      segmentId: segmentId,
      ...(seatClass && { class: seatClass }),
      "reservation.type": "temporary",
      "reservation.expiresAt": { $lt: Date.now() },
...
export async function isSeatTakenByElse(seatId, currentPassengerId) {
  const seat = await getOneDoc("FlightSeat", {
    _id: seatId,
    $or: [
      { "reservation.for": currentPassengerId },
...
export async function assignSeatsToFlightBooking(
  ...
  for (const segment of segments) {
    ...
    const seats = [];
    const availableSeats = await getAvailableSeats(segmentId);
