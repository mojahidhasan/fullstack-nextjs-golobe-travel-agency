"use server";
import { signIn, signOut as _signOut, signUp, auth } from "./auth";
...
export async function signUpAction(prevState, formData) {
  ...
  if (validate.success === true) {
    validate.data.image = await generateAvatar(validate.data.firstname);
...
export async function addOrRemoveLike({ isLiked, id, flightsOrHotels }) {
  const user = (await auth())?.user;
  if (!user) {
    return {
      error: "login_error",
