...
import { updateUser, updateUserDetails } from "./db/updateOperationDB";
...
export async function signUpAction(prevState, formData) {
  ...
  if (validate.success === true) {
    const generateAvatar = await generateAvatar(validate.data.firstname);
    validate.data.image = generateAvatar;
...
export async function sendPassResetCodeAction(prevState, formData) {
  const email = formData?.get("email") || cookies().get("ve");
  const schema = z
    .object({
      email: z
        .string()
  ...
  const verificationCode = +Math.random().toFixed(6).substring(2);
  console.log(verificationCode);

  try {
    await updateUser({ passwordResetCode: verificationCode });

    const htmlEmail = assignVars("passwordResetCode", {
      code: verificationCode,
      resetUrl:
        process.env.BASE_URL + "/api/verify?p_reset_v_code=" + verificationCode,
    });
    // await sendEmail(
    //   [schema.data.email],
    ...
    cookies().set("ve", schema.data.email, {
      maxAge: 15 * 60,
      httpOnly: true,
      secure: true,
    });
