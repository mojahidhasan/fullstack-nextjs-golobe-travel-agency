...
import { isRoomAvailable } from "../controllers/hotels";
...
export default async function hotelRoomReserveAction(bookingData) {
  ...
  const bookingObj = {
    ...
    payment: {
      method: null,
      status: "unpaid",
      transactionId: null, // This can be updated later after payment
    },
  };

  try {
    const booking = await createOneDoc("HotelBooking", bookingObj);
    return {
      success: true,
