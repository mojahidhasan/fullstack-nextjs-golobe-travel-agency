...
export async function anonymousUserSignUpAction() {
  const iAmLoggeIn = await isLoggedIn();
  const a_session = cookies().get("a_session"); // anonymous session
  if (iAmLoggeIn || a_session) {
    return {
      success: false,
  ...
  const expiry = Date.now() + 24 * 60 * 60 * 1000 * 15; // 15 days
  let created = null;
  try {
    const anonymousUser = await createOneDoc("AnonymousUser", {
      sessionId,
    ...
    if (created) {
      ...
      cookies().set("a_session", sessionId, {
        httpOnly: true,
        secure: true,
        sameSite: "strict",
      });
