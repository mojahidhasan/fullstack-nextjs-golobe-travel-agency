...
export async function flightReserveAction(prevState, formData) {
  ...
  try {
    ...
    const bookingObj = {
      ...
      totalPrice: Number((+metaData.totalPrice).toFixed(2)),
      paymentStatus: "pending",
      paymentId: null,
      seats: [],
      source: "website",
      temporaryReservationExpiresAt: reservationExpiresAt,
    ...
    if (!assignedSeats.length) {
      return {
        success: false,
        message:
          "No seats available of this flight, please try again later as some reserved seats may be released",
      };
    }
