...
export async function setPassengersDetailsAction(prevState, formData) {
  ...
  Object.entries(data).forEach(([key, value]) => {
    ...
    const { success, errors: e, data: d } = validatePassengerDetails(parsed);
    if (success === false) {
      errors[key] = e;
    }
    if (success === true) {
      validatedPassengers.push(d);
    }
  });
  ...
  try {
    ...
    const latestSearchState = userDetails.flights.latestSearchState;
    const nanoId = (nanoid() + Date.now().toString(36)).toUpperCase();
    const passengers = await createManyDocs(
      "Passenger",
      validatedPassengers.filter((p) => p.isPrimary === false),
    );
    const primaryPassenger = await createOneDoc(
      "Passenger",
      validatedPassengers.filter((p) => p.isPrimary === true)[0],
    );
    const bookingObj = {
      bookingRef: nanoId,
      userId: session.user.id,
      flightSnapshot: flight,
      tripType: latestSearchState.tripType,
      bookingStatus: "pending",
      passengers: [primaryPassenger, ...passengers],
      totalPrice: metaData.totalPrice,
      paymentStatus: "pending",
      seats: [],
      cabinClass: metaData.cabinClass,
      source: "website",
    };

    await createOneDoc("FlightBooking", bookingObj);
    return { success: true, message: "Passengers updated successfully" };
  } catch (error) {
    console.log(error);
